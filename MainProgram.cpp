...
public:
    ...
    BankAccount(const string& owner, double initialBalance) {
        ...
        if(owner.empty()){
            throw invalid_argument("owner boş olamaz!");
        }
        if(initialBalance < 0){
            throw invalid_argument("balance boş olamaz!");
        }
        owner_ = owner;
    ...
    void deposit(double amount) {
        // TODO: Implement
        if(amount <= 0){
            throw invalid_argument("Amount sıfırdan kucuk olamaz");
        }
        balance_ += amount; 
    }
    ...
    void withdraw(double amount) {
        // TODO: Implement
        if (amount <= 0){
            throw invalid_argument("Amount pozitif olmalı");
        }
        if(amount > balance_){
...
private:
    ...
    static void validate(const string& pwd) {
        ...
        if(pwd.length() < 8){
            throw invalid_argument("password 8 uzunlukta olmalı");
                }
        if(!hasDigit(pwd)){
            throw invalid_argument("password atlest must contain 1 digit");
                }
    }
...
public:
    ...
    void change(const string& oldPassword, const string& newPassword) {
        ...
        if(oldPassword != password_){
            throw invalid_argument("password is not matched");
        }
